...
export const commodityIntakesTable = boaSchema.table('commodity_intakes', {
  ...
  // transaction_type: 'trade' = available for sale/marketplace; 'storage' = held only
  transactionType: text('transaction_type', { enum: ['trade', 'storage'] }).notNull().default('trade'),
  // quality specs stored as JSON: {moistureContent?, foreignMatter?, pestDamage?, brokenGrains?}
  qualitySpecs:    text('quality_specs').notNull().default('{}'),
